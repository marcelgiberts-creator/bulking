...
async function loadProfile(){
  ...
  if(!p){
    p = { age: 23, height: 175, weight: 65, sex: 'm', activity: 1.2, goalOffset: 500, weeklyGainGoalKg: 0.3, mealsPerDay: 5, trainingDays: 4, preferences: '', targetKcal: null, targetProtein: null, targetCarbs: null, targetFat: null, lastAdjustmentWeek: null, adjustmentPaused: false, goalWeightKg: null, emaMaintenanceKcal: null };
  ...
  p.emaMaintenanceKcal = Number.isFinite(Number(p.emaMaintenanceKcal)) ? Number(p.emaMaintenanceKcal) : null;
  return p;
}
...
// Pendiente de regresión lineal simple (mínimos cuadrados) de un array de
// puntos {x, y}. Se usa para estimar la tendencia de peso/mantenimiento
// usando TODA la ventana de datos disponible en vez de solo los dos puntos
// extremos (mucho más robusto frente a un pesaje puntual ruidoso al
// principio o al final de la ventana). Devuelve null si no hay suficiente
// variación en el eje X para que la pendiente sea estable.
function linearRegressionSlope(points){
  ...
  let num = 0, den = 0;
  for(const p of points){ num += (p.x-meanX)*(p.y-meanY); den += (p.x-meanX)*(p.x-meanX); }
  if(den === 0) return null;
  return num / den; // unidades de y por unidad de x (ej. kg/día)
}
...
async function computeDynamicMaintenance(){
  ...
  // Regresión ponderada sobre TODA la ventana de la EMA (no solo el primer y
  // último punto): mucho más robusta frente a un pesaje puntual ruidoso justo
  // al principio o al final de la ventana, que antes podía distorsionar todo
  // el cálculo de mantenimiento por sí solo.
  const baseDate = new Date(first.date + 'T00:00:00');
  const regressionPoints = emaSeries.map(pt => ({ x: (new Date(pt.date + 'T00:00:00') - baseDate) / 86400000, y: pt.ema }));
  const slopePerDay = linearRegressionSlope(regressionPoints);
  const weightChangeKg = slopePerDay !== null ? slopePerDay * elapsedDays : (last.ema - first.ema);

  const impliedDailyBalance = (weightChangeKg * KCAL_PER_KG) / elapsedDays;
  const estimatedMaintenance = avgIntake - impliedDailyBalance;

  // Confianza (0-1): combina cantidad de pesajes y de días de comida
  // completos frente a un objetivo de ~14 de cada uno en la ventana de 21
  // días. Se usa para decidir cuánto puede moverse el objetivo esta semana
  // (más confianza → paso permitido más grande) y para mostrarlo en la UI.
  const confidence = Math.min(1, (dailyWeights.length / 14) * 0.5 + (intakeDays / 14) * 0.5);

  return { estimatedMaintenance, avgIntake, weightChangeKg, elapsedDays, intakeDays, weightDays: dailyWeights.length, confidence };
}
...
async function computeWeightProjection(){
  ...
  // Misma regresión ponderada que el modelo de mantenimiento: usa toda la
  // ventana en vez de solo el primer y último punto, así un pesaje atípico
  // en un extremo no dispara el ritmo estimado.
  const baseDate = new Date(first.date + 'T00:00:00');
  const regressionPoints = emaSeries.map(pt => ({ x: (new Date(pt.date + 'T00:00:00') - baseDate) / 86400000, y: pt.ema }));
  const slopePerDay = linearRegressionSlope(regressionPoints);
  const ratePerWeek = slopePerDay !== null ? slopePerDay * 7 : ((last.ema - first.ema) / elapsedDays) * 7;
...
async function adjustWeeklyTarget(){
  ...
  if(dynamic){
    // Suavizado recursivo (EWMA) del propio mantenimiento estimado: en vez de
    // fiarse por completo de la regresión de esta única semana (que sigue
    // teniendo ruido normal semana a semana), se mezcla con la creencia
    // acumulada de semanas anteriores. alpha=0.4 da más peso a lo nuevo que
    // a lo viejo, pero sin saltos bruscos por una semana atípica.
    const prevEmaMaintenance = Number.isFinite(Number(profile.emaMaintenanceKcal)) ? Number(profile.emaMaintenanceKcal) : dynamic.estimatedMaintenance;
    const smoothingAlpha = 0.4;
    const smoothedMaintenance = smoothingAlpha * dynamic.estimatedMaintenance + (1 - smoothingAlpha) * prevEmaMaintenance;
    profile.emaMaintenanceKcal = smoothedMaintenance;

    const desiredDailySurplus = (goal * KCAL_PER_KG) / 7;
    const rawTarget = smoothedMaintenance + desiredDailySurplus;
    const prevTarget = profile.targetKcal;
    // El paso máximo semanal permitido ya no es fijo: con más pesajes y más
    // días de comida completos (mayor confianza), se permite moverse más;
    // con datos escasos o irregulares, el objetivo se mueve con más cautela.
    const maxWeeklyStep = Math.round(100 + dynamic.confidence * 150); // rango 100-250 kcal
    ...
    await recordTargetAdjustment({
      ...
      note: `Mantenimiento real estimado (regresión de esta semana) ~${Math.round(dynamic.estimatedMaintenance)} kcal/día, suavizado con semanas anteriores a ~${Math.round(smoothedMaintenance)} kcal/día (${dynamic.weightDays} pesajes, ${dynamic.intakeDays} días de comida en ${dynamic.elapsedDays} días, confianza ${(dynamic.confidence*100).toFixed(0)}%, paso máximo ±${maxWeeklyStep} kcal).`
    });
    showAdjustAlert(`🧠 Modelo dinámico (confianza ${(dynamic.confidence*100).toFixed(0)}%, ${dynamic.weightDays} pesajes, ${dynamic.intakeDays} días de comida): mantenimiento real estimado ~${Math.round(smoothedMaintenance)} kcal/día. Objetivo ajustado a ${finalTarget} kcal (antes ${Math.round(prevTarget)}).`);
...
// Única llamada: identifica el alimento, estima gramos con las anclas de
// ración de arriba, y calcula kcal/macros con tablas de composición
// estándar españolas, todo en un mismo prompt/JSON.
async function estimateWithAIOnly(text){
  const prompt = `Actúa como Dietista Clínico español experto en estimar raciones de comida casera a partir de descripciones rápidas (dictadas por voz o texteadas deprisa, a menudo sin cantidad exacta).

Texto del registro: "${text}"

Referencias de ración habituales en cocina casera española (úsalas para estimar gramos cuando el texto no da cantidad exacta): ${PORTION_ANCHORS}

Pasos a seguir mentalmente (no los muestres, solo da el resultado final):
1. Identifica cada alimento/ingrediente mencionado y una cantidad razonable en gramos para cada uno (usa las referencias de arriba si no se especifica cantidad; si hay varias piezas o raciones, multiplica en consecuencia).
2. Aplica tablas de composición nutricional estándar españolas por alimento.
3. Suma el total de kcal y macros de todos los alimentos del texto.

Incluye "s" (azúcares totales en g): 0 si es un alimento sin azúcar (pollo, brócoli, aceite...), valor real si lleva fruta, lácteos, miel, salsas o ultraprocesados.
Si el texto no describe comida real, pon todos los valores numéricos a 0 y explica brevemente por qué en "reply".

Devuelve SOLO JSON estricto, sin texto ni markdown fuera de él:
{"label":"Nombre resumido de la comida","kcal":numero,"p":numero,"c":numero,"f":numero,"s":numero,"reply":"mensaje corto motivador"}`;
  const res = await callGemini(prompt, true, GEMINI_MODEL);
  if(res){ res.source = 'Estimación de IA'; }
  return res;
}

// Envoltorio con caché: si el texto normalizado ya se estimó antes, reutiliza
// el resultado sin llamar a la IA otra vez (rápido para comidas repetidas
// dictadas con las mismas palabras). Si no, hace la única llamada a Gemini
// y guarda el resultado para la próxima vez.
async function estimateFoodEntry(text){
  const cacheKey = normalizeFoodKey(text);
  const cached = cacheKey ? await getFoodCache(cacheKey) : null;
  if(cached && cached.result && validateFoodEntry(cached.result)){
    return { ...cached.result, source: (cached.result.source || 'Estimación de IA') + ' (caché)' };
  }
  const result = await estimateWithAIOnly(text);
  if(result && validateFoodEntry(result) && result.kcal > 0 && cacheKey){
    await setFoodCache(cacheKey, { result });
  }
  return result;
}
...
// Etiqueta de origen de la estimación (ya no hay niveles de fiabilidad
// distintos: todo pasa por el mismo modelo de IA, con o sin caché).
function reliabilityBadge(source){
  if(!source) return '';
  const cached = /caché/i.test(source);
  const color = cached ? '#60a5fa' : 'var(--accent)';
  const label = cached ? '⚡ Reutilizado de un registro anterior' : '🤖 Estimación de IA';
  return `<div style="display:inline-flex; align-items:center; gap:6px; margin-top:8px; padding:5px 10px; border-radius:20px; background:rgba(255,255,255,0.06); font-size:.72rem; font-weight:700; color:${color};">${label}</div>`;
}
...
function showFoodReview(entry, isEdit=false){
  ...
  review.innerHTML = `<strong>${isEdit ? 'Editar registro' : 'Revisa antes de guardar'}</strong><div style="color:var(--text-dim);font-size:.8rem;margin-top:4px;">${isEdit ? 'Corrige los valores y confirma (pulsa el número para escribir encima).' : 'La IA ha estimado estos valores. Pulsa para editarlos.'}</div>${reliabilityBadge(entry.source)}
  ...
  review.innerHTML = `<strong>Nueva estimación</strong>
    ...
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin:14px 0; font-size:.82rem;">
      ...
      <div style="padding:10px; border-radius:8px; background:rgba(16,185,129,0.08); border:1px solid rgba(16,185,129,0.25);">
        ...
        <div style="margin-top:6px;">${reliabilityBadge(newEntry.source)}</div>
      </div>
    </div>
...
window.reestimateLog = async (id) => {
  ...
  // Llama directamente al modelo (sin pasar por el caché de estimateFoodEntry):
  // re-estimar solo tiene sentido si es una pasada genuinamente nueva, no si
  // te devuelve lo mismo que ya tenías guardado en caché.
  ...
  const cacheKey = normalizeFoodKey(entry.originalText);
  if(cacheKey) await setFoodCache(cacheKey, { result: res }); // refresca el caché con la nueva pasada

  const kcalDiff = res.kcal - entry.kcal;
  const pctDiff = entry.kcal > 0 ? Math.abs(kcalDiff) / entry.kcal * 100 : 100;
  if(pctDiff < 3){
    showToast(`Sin cambios relevantes: la nueva estimación da un resultado muy similar (diferencia de ${Math.round(Math.abs(kcalDiff))} kcal, <3%).`, false);
    return;
  }
...
async function processText(voiceText){
  ...
  $('ai-status').innerText = 'Estimando kcal y macros con la IA...';
...
async function sendChatMessage(){
  ...
  // Contexto COMPLETO de hoy: mismos datos exactos que ve el asistente de la
  // pestaña "Hoy" (kcal/macros objetivo, ingeridos y restantes, comidas ya
  // registradas, hora del día e histórico reciente), para que el coach
  // pueda responder con conocimiento real de la situación, no a ciegas.
  const t = todayStr();
  ...
  const prompt = `Eres coach nutricionista del usuario en la app Bulking OS (nutrición para volumen/ganancia muscular). Habla directo, clínico pero amistoso, y usa los datos reales de abajo en vez de suposiciones genéricas.

Perfil: peso actual ${profile.weight} kg, meta de ganancia ${profile.weeklyGainGoalKg || 0.3} kg/semana, kcal base ${Math.round(profile.targetKcal||2500)}${overrideToday ? ` (con un ajuste manual ya aplicado hoy de ${overrideToday>0?'+':''}${overrideToday} kcal)` : ''}.
${dyn ? `Mantenimiento calórico real estimado (modelo dinámico): ~${Math.round(dyn.estimatedMaintenance)} kcal/día.` : ''}

${statusText}

Instrucciones:
- Usa las kcal/macros restantes y las comidas ya registradas hoy para responder con precisión (ej. si pregunta si le cabe algo, compáralo con lo que le queda de verdad).
...
async function generateBodySummary(){
  ...
  const insights = await computeWeekInsights();
  const targetHist = ((await safeGet('targetHistory')) || []).slice(-3);
  const targetHistText = targetHist.length
    ? `- Últimos ajustes de objetivo: ${targetHist.map(h=>`semana ${h.week}: ${Math.round(h.prevTarget)}→${Math.round(h.newTarget)} kcal (${h.mode==='dinamico'?'modelo dinámico':'básico'})`).join('; ')}.`
    : '- Aún no se ha completado ningún ajuste semanal automático.';
...
${trend.status === 'ok' ? `- Ritmo real de cambio de peso: ${trend.ratePerWeek >= 0 ? '+' : ''}${trend.ratePerWeek.toFixed(2)} kg/semana (${trend.dataPoints} pesajes en ${trend.elapsedDays} días, confianza ${trend.confidence}).` : '- Sin histórico de peso suficiente todavía.'}
${dyn ? `- Mantenimiento calórico real estimado (regresión ponderada): ${Math.round(dyn.estimatedMaintenance)} kcal/día (confianza ${(dyn.confidence*100).toFixed(0)}%).` : ''}
- Objetivo de kcal actual: ${Math.round(profile.targetKcal || 0)} kcal/día. Entrenamientos/semana: ${profile.trainingDays || 0}.
- Adherencia últimos 7 días: ${insights.kcalDays}/${insights.completeDays || 7} días dentro de ±10% del objetivo de kcal, ${insights.proteinDays}/${insights.completeDays || 7} días con proteína ≥90% del objetivo. Media real ingerida: ${insights.avgKcal ? insights.avgKcal + ' kcal/día' : 'sin registros suficientes'}.
${targetHistText}
- Ajuste automático semanal: ${profile.adjustmentPaused ? 'pausado por el usuario' : 'activo'}.
...
async function renderDynamicModelStatus(){
  ...
  const smoothed = Number.isFinite(Number(profile.emaMaintenanceKcal)) ? Number(profile.emaMaintenanceKcal) : dyn.estimatedMaintenance;
  const diff = smoothed - formulaTdee;
  const confColor = dyn.confidence >= 0.7 ? 'var(--green)' : dyn.confidence >= 0.4 ? 'var(--accent)' : 'var(--red)';
  el.innerHTML = `
    <div style="padding:12px; border-radius:var(--radius-sm); background:rgba(16,185,129,0.08); border:1px solid rgba(16,185,129,0.25); margin-bottom:12px; font-size:0.82rem; color:#d1fae5;">✅ Modelo dinámico activo (${dyn.weightDays} pesajes, ${dyn.intakeDays} días de comida) · confianza <b style="color:${confColor};">${(dyn.confidence*100).toFixed(0)}%</b></div>
    <div style="display:flex; justify-content:space-between; padding-bottom:10px; border-bottom:1px solid var(--glass-border); margin-bottom:10px;"><span style="color:var(--text-dim);">Mantenimiento estimado (suavizado)</span><b>${Math.round(smoothed)} kcal</b></div>
    <div style="display:flex; justify-content:space-between; padding-bottom:10px; border-bottom:1px solid var(--glass-border); margin-bottom:10px;"><span style="color:var(--text-dim);">Regresión de esta semana (sin suavizar)</span><b>${Math.round(dyn.estimatedMaintenance)} kcal</b></div>
...
// Calcula la adherencia de los últimos 7 días (kcal, proteína, media real).
// Función pura sin efectos DOM para que también pueda alimentar el prompt
// del resumen clínico de IA, no solo el render del dashboard.
async function computeWeekInsights(){
  const avgWeight = await getAverageWeight(0, 7);
  let kcalDays = 0, proteinDays = 0, completeDays = 0, kcalTotal = 0;
...
window.runSelfTests = async function(){
  ...
  // --- Regresión lineal (modelo de mantenimiento dinámico) ---
  const perfectLine = [{x:0,y:10},{x:1,y:12},{x:2,y:14},{x:3,y:16}]; // pendiente exacta = 2
  check('Regresión: pendiente exacta en línea perfecta (=2)', approx(linearRegressionSlope(perfectLine), 2, 0.001));
  const noisyLine = [{x:0,y:10},{x:1,y:11.5},{x:2,y:14.5},{x:3,y:15.5}]; // pendiente ~1.83, con ruido
  const noisySlope = linearRegressionSlope(noisyLine);
  check('Regresión: pendiente razonable con ruido (entre 1 y 3)', noisySlope > 1 && noisySlope < 3);
  check('Regresión: null con menos de 2 puntos', linearRegressionSlope([{x:0,y:5}]) === null);
